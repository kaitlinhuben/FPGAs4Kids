...
Elm.Shanghai.make = function (_elm) {
   ...
   var updateDocks = F2(function (ship,
   docks) {
      return function () {
         switch (ship.ctor)
         {case "Err":
   ...
   var outgoingShip = _P.portIn("outgoingShip",
   _P.incomingSignal(function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",
      v);
   }));
   var incomingShip = _P.portIn("incomingShip",
   _P.incomingSignal(function (v) {
      return typeof v === "object" && "name" in v && "capacity" in v ? {_: {}
                                                                       ...
                                                                       v.name)
                                                                       ,capacity: typeof v.capacity === "number" ? v.capacity : _U.badPort("a number",
                                                                       v.capacity)} : _U.badPort("an object with fields \'name\', \'capacity\'",
      v);
   }));
   var ships = A2($Signal.merge,
   A2($Signal._op["<~"],
   $Result.Ok,
   incomingShip),
   A2($Signal._op["<~"],
   $Result.Err,
   outgoingShip));
   var dock = A3($Signal.foldp,
   updateDocks,
   $Dict.empty,
   ships);
   var totalCapacity = _P.portOut("totalCapacity",
   _P.outgoingSignal(function (v) {
      return v;
   }),
   A2($Signal._op["<~"],
   function ($) {
      return $List.sum($Dict.values($));
   },
   dock));
   var coordinates = _P.portIn("coordinates",
   function (v) {
